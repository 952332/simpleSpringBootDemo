			...
			<div class="container">
				...
				<div class="row row_det">
						<div v-if="this.$store.state.user.user_id" class="btns_add col-12 col-sm-6">
							<div v-if="$check_action('/ordinary_user/details','set')">
								<!-- 收藏按钮 -->
...
<script>
	...
	export default {
		...
		methods: {
			...
			change_collect() {
				...
				var query = {
					...
					source_field: "ordinary_user_id",
					source_id: this.obj["ordinary_user_id"],
					user_id
				};
      ...
      check_collect() {
        ...
        var ordinary_user_id = this.obj.ordinary_user_id;
        this.$get('~/api/collect/count?', {
          user_id,
          source_table: "ordinary_user",
          source_field: "ordinary_user_id",
          source_id: ordinary_user_id
        }, (res) => {
